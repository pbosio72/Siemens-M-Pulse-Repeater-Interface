...
The system uses an MCP23017 I2C expander to handle the keypad matrix.
...
- 3x4 Matrix Keypad connected to MCP23017 (PA0-PA6)
...
Physical Pin → MCP23017 Pin → Function
Pin 1        → PA0          → Column 2 (2, 5, 8, 0)
Pin 2        → PA1          → Row 1 (1, 2, 3)
Pin 3        → PA2          → Column 1 (1, 4, 7, *)
Pin 4        → PA3          → Row 4 (*, 0, #)
Pin 5        → PA4          → Column 3 (3, 6, 9, #)
Pin 6        → PA5          → Row 3 (7, 8, 9)
Pin 7        → PA6          → Row 2 (4, 5, 6)
...
const int delay_huge = 8000;       // Extended delay for final results
...
// MCP23017 I2C expander (I2C address 0x20)
...
// MCP23017 pin assignments (based on physical testing)
...
char scanKeypad() {
  ...
  for (int r = 0; r < ROWS; r++) {
    // Set current row LOW (active), all others HIGH (inactive)
    for (int i = 0; i < ROWS; i++) {
      mcp.digitalWrite(rowPins[i], (i == r) ? LOW : HIGH);
    }
...
void setup() {
  ...
  if (!mcp.begin_I2C(0x20)) {
    ...
    while (1); // Halt execution if MCP23017 fails
  }
   
  // Configure MCP23017 pins for keypad matrix
  // Rows as outputs (will be driven LOW one at a time)
  for (int i = 0; i < ROWS; i++) {
    mcp.pinMode(rowPins[i], OUTPUT);
    mcp.digitalWrite(rowPins[i], HIGH); // Start with all rows HIGH
  }
  // Columns as inputs with pull-ups (will read LOW when button pressed)
