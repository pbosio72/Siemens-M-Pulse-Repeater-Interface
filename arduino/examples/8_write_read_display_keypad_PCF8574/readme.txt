...
Sketch Name: 8_write_read_display_keypad_PCF8574.ino
Version: 1.0
Date: 2026-02-01
Description: Complete mechanical memory system with 3x4 keypad input via PCF8574
...
and stored mechanically on a rotating disk. The system uses a PCF8574 I2C expander
to handle the keypad matrix, freeing up Arduino pins for other components.

Key Features:
- 3x4 matrix keypad input via PCF8574 I2C expander (8 I/O pins)
...
- Uses PCF8574 I2C expander (simpler and cheaper than MCP23017)
- Implements custom keypad scanning function with bitwise operations
...
ARDUINO UNO:
  Digital Pin 6  → Switch A (reserved for future use)
  Digital Pin 7  → Switch B (reserved for future use)
  Digital Pin 8  → Switch C (CCW limit sensor)
...
WHY PCF8574 INSTEAD OF MCP23017:
The PCF8574 is more suitable for this project because:
- 8 I/O pins are sufficient (keypad needs only 7 pins)
- Simpler to use (no pin mode configuration needed)
- More cost-effective (~1€ vs ~2-3€)
- Lower power consumption
- Adequate speed for keypad scanning
- Built-in pull-up resistors
The MCP23017 (16 I/O) would be oversized for this application.
...
4. Search for "PCF8574" and install (by Rob Tillaart)
...
The system uses matrix scanning to detect keypresses with PCF8574:
1. All 8 pins are written simultaneously using pcf.write8()
2. Each row is activated (set LOW) one at a time using bitwise operations
3. All columns are read simultaneously using pcf.read8()
4. If a column reads LOW while a row is active, that key is pressed
5. Key position is determined by row-column intersection
6. Debouncing prevents multiple triggers from single press

PCF8574 ADVANTAGES FOR KEYPAD SCANNING:
- Built-in pull-up resistors (no external resistors needed)
- 8-bit read/write operations are atomic (no timing issues)
- Simple bitwise manipulation for row activation
- Adequate speed for human input (no latency issues)
- Interrupt capability available (not used in this implementation)
...
Unlike previous versions that stored digits then wrote them all at once,
this version writes each digit to memory immediately after entry:
...
This prevents data loss and provides instant feedback.
...
"PCF8574 ERROR!"          - PCF8574 not detected (halts execution)
...
I2C Addresses:
  ...
  PCF8574:  0x20 (default, configurable with A0/A1/A2 jumpers)

To change LCD address (if 0x27 doesn't work):
  LiquidCrystal_I2C lcd(0x3F, 20, 4);

To change PCF8574 address:
  PCF8574 pcf(0x21);  // Change 0x20 to desired address
...
The scanKeypad() function implements matrix scanning with PCF8574:
1. Prepare byte pattern with all pins HIGH except current row
2. Write pattern to PCF8574 using pcf.write8()
3. Read all pins simultaneously using pcf.read8()
4. Check column pins using bitRead() to detect LOW state
5. If column is LOW, key at (row, column) is pressed
6. Return corresponding character from keys[][] array
7. If no key pressed, return 0

PCF8574 BITWISE OPERATIONS:
  byte rowPattern = 0xFF;              // All pins HIGH (11111111)
  bitClear(rowPattern, rowPins[r]);   // Set row pin LOW
  pcf.write8(rowPattern);             // Write to all 8 pins at once
  byte readValue = pcf.read8();       // Read all 8 pins at once
  if (bitRead(readValue, colPins[c]) == LOW)  // Check column pin

This approach is more efficient than individual pinMode/digitalWrite calls.
...
Problem: "PCF8574 ERROR!" on LCD at startup
Solution:
  - Check PCF8574 power connections (VCC to 5V, GND to GND)
  - Verify I2C connections (SDA to A4, SCL to A5 on Uno)
  - Check I2C address (default 0x20)
  - Run I2C scanner sketch to detect PCF8574
  - Verify PCF8574 is properly seated if using breadboard
  - Check for solder bridges if using custom PCB
  - Verify A0/A1/A2 address pins configuration

Problem: LCD shows text but no keypad response
Solution:
  - Verify keypad is connected to correct PCF8574 pins
  - Check rowPins[] and colPins[] arrays match your keypad
  - Use multimeter to test keypad pin mapping
  - Add debug Serial.print() to scanKeypad() function
  - Test PCF8574 with simple write8/read8 operations
  - Verify built-in pull-ups are working (no external needed)
...
Solution:
  ...
  - Add small capacitors (0.1µF) across keypad pins if needed
  - PCF8574 has built-in pull-ups, no external resistors needed
  - Reduce scan speed (increase delay between pcf.write8() and pcf.read8())
...
Setup Section:
- Initialize all pins (Arduino digital pins)
- Initialize LCD2004 display
- Initialize PCF8574 I2C expander
- Set all PCF8574 pins HIGH initially (ready for keypad scanning)
...
- PCF8574 shares I2C bus with LCD - different addresses prevent conflicts
- PCF8574 has built-in pull-up resistors - no external resistors needed
...
- Test PCF8574 separately with simple I2C scanner
...
3. Should detect devices at 0x20 (PCF8574) and 0x27 (LCD)
